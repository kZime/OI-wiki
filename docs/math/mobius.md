...
* 由唯一分解定理设 $n = \prod_{i=1}^{s}p_i^{k_i}$ ，其中 $p_i$ 是质数，那么 $\varphi(n) = n \times \prod_{i = 1}^s{\dfrac{p_i - 1}{p_i}}$ 。
  ...
  * 引理（1）：设 $x=p^k$ , 那么 $\varphi(x)=p^{k-1}\times(p-1)$ 。
    ...
    容易发现 $x\perp y (y\bmod p \ne 0)$ 。我们试着将 $x$ 划分为长度为 $p$ 的 $\dfrac{p^k}{p}=p^{k-1}$ 段，每一段都有 $p-1$ 个数与 $x$ 互质。所以与 $x$ 互质的数个数即为： $p^{k-1}\times(p-1)$， 证毕。

  接下来我们证明 $\varphi(n) = n \times \prod_{i = 1}^s{\dfrac{p_i - 1}{p_i}}$ 。

  由$n = \prod_{i=1}^{s}p_i^{k_i}$以及$\varphi(x)$函数的积性我们有

$$
\begin{aligned}
	\varphi(n) &= \prod_{i=1}^{s} 
	\varphi(p_i^{k_i}) \\
